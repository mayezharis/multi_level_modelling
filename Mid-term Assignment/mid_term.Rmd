...
relevant_cols <- c("Country", "indiv", "gender", "age", "eduyrs", "Universalism")
ess_clean <- ess_data %>% 
...
```{r including gender into previous model}
ri2 <- lmer(Universalism ~ agecen + gender + eduyrs_cen + (1+gender|Country), data = ess_q2)
summary(ri2)
```

```{r plotting gender model}
...
ess_final <- ess_q2 %>% mutate(age_binned = cut(age, breaks=c(min(age), 25, 35, 50, 65, max(age))))
...
```{r Final model}
ri3 <- lmer(Universalism ~ age_binned + gender + eduyrs_cen + (1+age_binned|Country), data = ess_final)
summary(ri3)
```

```{r final model dotplot}
ri3_plot_data <- as.data.frame(ranef(ri3))
ri3_dotplot <- ri3_plot_data %>% 
  ggplot(aes(x=condval, y=grp, color = term)) +
  geom_dotplot(binaxis="y", stackdir="center", stackratio=1.5,dotsize=0.5) +
  geom_errorbar(aes(xmin=condval + (qnorm(0.025) * condsd), 
                    xmax =condval + (qnorm(0.975) * condsd)),
  ...
  facet_grid(.~term, 
             labeller = labeller(term = c("(Intercept)" = "Intercept", 
                                          "age_binned(25,35]" = "Ages 26-35",
                                          "age_binned(35,50]" = "Ages 36-50",
                                          "age_binned(50,65]" = "Ages 51-65",
                                          "age_binned(65,102]" = "Ages 66 and over")))
```

```{r}
vcm_validation <- lmer(Universalism ~ (1|Country), data = ess_q2, REML=FALSE)

anova(vcm_validation, ri1)
```
```{r}
ri1_validation <- lmer(Universalism ~ age_binned + gender + eduyrs_cen + (1|Country), data = ess_final)
anova(ri1_validation, ri3)
```
