...
sex_boxplot <-age_uni %>%
  ggplot(aes(x = gender, y = Universalism, fill = gender)) +
  geom_boxplot() + 
  labs(x = "Sex of Individual",
       y = "Universalism Score") +
  theme(axis.text=element_text(size=17),
        axis.title=element_text(size=17),
        legend.title = element_text(size = 17),
...
fit2 <- lm(Universalism ~ agecen:female, data = age_uni)
...
age_uni_fit_plot <- age_uni %>%
  ggplot(aes(x = agecen, y = Universalism)) +
  geom_point(alpha = 0.5, color = "cornflowerblue") +
  geom_hline(yintercept = 0, linetype = 2, linewidth = 0.75) +
  labs(title = "Universalism by Age",
       x = "(Centred) Age of Individual", 
       y = "Universalism Score") +
  stat_smooth(method = "lm", color = "red2")
